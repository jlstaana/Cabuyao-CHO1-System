  ...
  return (
    <div className="flex flex-col lg:flex-row lg:h-[calc(100vh-8rem)] gap-6 animate-in fade-in duration-500">      {/* Video Call Area */}
      ...
      <div className="w-full lg:w-[28rem] flex flex-col gap-4 lg:overflow-y-auto lg:pr-2 custom-scrollbar">
            ...
            {user.role === 'Patient' ? (
               ...
               <div className="space-y-2 text-sm">
                  {consultation?.vital_signs ? (
                    <>
                      <div className="flex justify-between border-b border-slate-50 pb-2"><span className="text-text-muted">BP</span><span className="font-medium">{consultation.vital_signs.blood_pressure}</span></div>
                      <div className="flex justify-between border-b border-slate-50 pb-2"><span className="text-text-muted">Heart Rate</span><span className="font-medium">{consultation.vital_signs.heart_rate} bpm</span></div>
                      <div className="flex justify-between pb-2"><span className="text-text-muted">Temperature</span><span className="font-medium">{consultation.vital_signs.temperature} °C</span></div>
                    </>
                  ) : <p className="text-text-light italic">Waiting for patient to submit vitals...</p>}
               </div>
            )}
         </div>

         {/* Session Chat */}
         <div data-tour="page-chat" className="bg-surface rounded-2xl p-5 shadow-sm border border-border shrink-0">
            ...
            <div ref={chatListRef} className="h-56 overflow-y-auto rounded-xl border border-border bg-background p-3 space-y-3 custom-scrollbar">
              {chatMessages.length === 0 ? (
                <p className="text-xs text-text-light text-center py-16">No messages yet.</p>
              ) : chatMessages.map((message) => {
                const isMine = message.sender_id === user?.id;
                return (
                  <div key={message.id} className={`flex ${isMine ? 'justify-end' : 'justify-start'}`}>
                    <div className={`max-w-[82%] rounded-2xl px-3 py-2 text-sm ${isMine ? 'bg-teal-500 text-white rounded-br-md' : 'bg-surface text-slate-700 border border-border rounded-bl-md'}`}>
                      <div className={`mb-1 text-[10px] font-semibold ${isMine ? 'text-teal-50' : 'text-text-light'}`}>
                        {isMine ? 'You' : message.sender?.name || 'Participant'}
                      </div>
                      <p className="whitespace-pre-wrap break-words leading-relaxed">{message.message}</p>
                      <div className={`mt-1 text-[10px] ${isMine ? 'text-teal-50' : 'text-text-light'}`}>
            ...
            <form onSubmit={sendChatMessage} className="mt-3 flex gap-2">
              <input
                ...
                className="min-w-0 flex-1 px-4 py-2 rounded-xl border border-border text-sm focus:ring-2 focus:ring-teal-500/20 outline-none"
                maxLength={1000}
              />
         ...
         {/* Medical Images */}
         <div data-tour="page-list" className="bg-surface rounded-2xl p-5 shadow-sm border border-border shrink-0">
            ...
            {user.role === 'Patient' && (
               <div className="border-2 border-dashed border-border rounded-xl p-4 text-center hover:bg-background transition-colors cursor-pointer mb-4" onClick={() => navigate('/medical-images')}>
                  <Upload size={24} className="mx-auto text-text-light mb-2" />
         ...
         {user.role === 'Doctor' && (
            <div data-tour="page-prescription" className="bg-surface rounded-2xl p-5 shadow-sm border border-border flex-1 flex flex-col">
               ...
               <form className="space-y-4 flex-1 flex flex-col" onSubmit={completeConsultation}>
                  {/* Diagnosis */}
                  <textarea value={symptoms} onChange={e=>setSymptoms(e.target.value)} placeholder="Observed symptoms..." className="w-full px-4 py-2 rounded-xl border border-border text-sm focus:ring-2 focus:ring-emerald-500/20 outline-none resize-none h-16 shrink-0" required />
                  <textarea value={diagnosis} onChange={e=>setDiagnosis(e.target.value)} placeholder="Official Diagnosis..." className="w-full px-4 py-2 rounded-xl border border-border text-sm focus:ring-2 focus:ring-emerald-500/20 outline-none resize-none h-16 shrink-0" required />
                     ...
                     <div className="space-y-3">
                       ...
                       {prescriptionItems.map((item, idx) => (
                         <div key={idx} className="bg-surface p-2 rounded-lg border border-border space-y-2">
                            <select 
                              className="w-full text-sm p-1.5 border border-border rounded bg-background"
                            ...
                            <div className="flex gap-2">
                               <input placeholder="Dosage (e.g. 1 Tablet)" value={item.dosage} onChange={e => { const newItems = [...prescriptionItems]; newItems[idx].dosage = e.target.value; setPrescriptionItems(newItems); }} className="w-1/2 text-xs p-1.5 border border-border rounded" required/>
                               <input placeholder="Frequency (e.g. 3x a day)" value={item.frequency} onChange={e => { const newItems = [...prescriptionItems]; newItems[idx].frequency = e.target.value; setPrescriptionItems(newItems); }} className="w-1/2 text-xs p-1.5 border border-border rounded" required/>
                            </div>
                         </div>
                       ))}
                     </div>
                  ...
                  {prescriptionItems.length > 0 && (
                    <div className="border border-border rounded-xl bg-surface p-4">
                      <div className="flex items-center justify-between gap-3 mb-3">
                        ...
                        <button type="button" onClick={clearSignature} className="text-xs bg-surface-hover/50 text-text-muted px-2 py-1 rounded hover:bg-slate-200 dark:hover:bg-zinc-800 font-bold flex items-center gap-1">
