...
const replacement = `
        ...
        if (isset($data['exceptions'])) {
            ...
            foreach ($data['exceptions'] as $exc) {
                $doctor->exceptions()->create([
                    'date' => $exc['date'],
                    'type' => $exc['type'],
                    'start_time' => $exc['type'] === 'extra_slot' ? $exc['start_time'] : null,
                    'end_time' => $exc['type'] === 'extra_slot' ? $exc['end_time'] : null,
                ]);
            }
        }
        ...
        $msg = 'Availability settings updated successfully.';
        if ($reassignedCount || $rescheduledCount) {
            $msg .= " Emergency Protocol Triggered: {$reassignedCount} patient(s) reassigned to substitutes, {$rescheduledCount} patient(s) auto-rescheduled.";
        }
...
const regex = /\$doctor->availability\(\)->delete\(\);[\s\S]*?return response\(\)->json\(\['message' => 'Availability settings updated successfully\.'\]\);\s*\}/;
