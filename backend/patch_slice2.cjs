...
if (idxStart !== -1) {
  ...
  const endMarker = '</div>';
  let curr = f.indexOf(endMarker, idxStart);
  curr = f.indexOf(endMarker, curr + 1); // outer div closing
  ...
  if (curr !== -1) {
    ...
    f = f.slice(0, actualStart) + newChart + f.slice(curr + 6);
    fs.writeFileSync('../frontend/src/pages/dashboard/Analytics.jsx', f);
    console.log('Fixed System Utilization completely');
  }
}
